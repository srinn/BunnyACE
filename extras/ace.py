...
class BunnyAce:
    ...
    def _park_to_toolhead(self, tool):
        ...
        self._feed(tool, self.toolchange_retract_length + (1000 if sensor_toolhead is not None else 200), self.retract_speed, 50)
        ...
        while not bool(sensor_extruder.runout_helper.filament_present):
            ...
            if self._info['status'] == 'ready':
                ...
                self._feed(tool, 100, self.retract_speed, 1)
            self.dwell(delay=0.01)

        self.gcode.respond_info('ACE: set feed speed to 10')
        self._set_feeding_speed(tool, 5)
        # # self._set_feeding_speed(tool, 10)
    ...
    def _reject_tool(self, index):
        ...
        sensor_extruder = self.printer.lookup_object('filament_switch_sensor %s' % 'extruder_sensor', None)
        sensor_splitter = self.printer.lookup_object(f'filament_switch_sensor splitter_t{index}_sensor', None)
        ...
        if self.variables.get('ace_filament_pos', "spliter") == "toolhead":
            ...
            if sensor_splitter:
                ...
                self._retract(index, self.toolchange_retract_length + 100, 10, 1)
            while bool(sensor_extruder.runout_helper.filament_present):
                # self.gcode.respond_info('ACE: check extruder sensor')
                if self._info['status'] == 'ready':
                    self._retract(index, 200, 10, 1)
                self._extruder_move(-5, 10)
                self.dwell(delay=0.01)
        ...
        self._set_retracting_speed(index, self.retract_speed)
        if sensor_splitter:
            while bool(sensor_splitter.runout_helper.filament_present):
                if self._info['status'] == 'ready':
                    self._retract(index, 9999, 10, 1)
                self.dwell(delay=0.01)
            self._stop_retracting(index)
        # self.wait_ace_ready()
